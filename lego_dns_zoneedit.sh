...
# Used by lego  exec provider. that is alos used by traefik to create  certificates
#
########  Public functions #####################

# Usage: lego_dns_zoneedit.sh _acme-challenge.www.domain.com   "XKrxpRBosdIKFzxW_CT3KLZNf6q0HG9i01zxXp5CPBs"
# the info between the qoutation  is the letsencrypt token.
...
_readaccountconf_mutable(){
  item=$1
  if [ $item = "ZONEEDIT_ID" ]; then
    res="dummy_id"
  else if [ $item = "ZONEEDIT_Token" ]; then 
    res="dummy_token"
  else
    res="error"
  fi
...
dns_zoneedit_add() {
  ...
  # Load the credentials from th envronmente
  ...
  if [ -z "$ZONEEDIT_ID" ] || [ -z "$ZONEEDIT_Token" ]; then
    ZONEEDIT_ID=""
    ZONEEDIT_Token=""
    _err "Please specify ZONEEDIT_ID and ZONEEDIT_Token."
